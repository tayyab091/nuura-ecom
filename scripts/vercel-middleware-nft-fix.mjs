...
// Some Vercel packaging paths assume both files exist.
// Create a harmless stub `middleware.js` if Next didn't emit one.
if (!fileExists(middlewareJsPath)) {
  fs.mkdirSync(path.dirname(middlewareJsPath), { recursive: true })
  fs.writeFileSync(
    middlewareJsPath,
    [
      '/* eslint-disable */',
      '// Auto-generated by postbuild to satisfy Vercel file tracing.',
      '// Next 16 + Turbopack outputs middleware as Edge chunks under `.next/server/edge/`.',
      '// This stub should never be executed in production on Vercel; it only prevents ENOENT during packaging.',
      'module.exports = {}',
      '',
    ].join('\n'),
  )
...
// Always include the stub so Vercel can trace it if needed.
